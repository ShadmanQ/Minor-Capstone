...
class initialPage(Frame):
    ...
    def create_widgets(self):
        ...
        #stores the name of the compound as a local variable to be accessed later
    ...
    #function which accesses the ChemSpider API and retrieves results based on the search term
    def SearchBox(self):
    ...
    def get_list(self,confirmLabel):
        ...
        #once a compound is selected, the following buttons are generated to create
...
# window which retrieves and downloads Spectra from ChemSpider
class spectraWindow(Frame):
    ...
    def create_spectraWindow(self,frame):
        ...
        #sorts out IR and HNMR spectra, the most relevant spectra for undergrad Chem students
...
class propertyWindow(Frame):
    def __init__(self,master,number):
        ...
        self.index = number
        #preloaded messages on the off-chance this date is unavailable in the database
        ...
        #utilizes the unique ID of each compound in the database to generate a url which is then scraped by requests library
    ...
    def create_propertyWindow(self,frame):
        ...
        #data in webpage is presented as an unordered list, this function extracts all ul's from webpage
        f_data = soup.find_all("ul")

        #all webpage data is stored in a local dictionary
        ...
        for x in range(0,len(ChemData)):
            if "Melting Point" in ChemData[x]:
                ...
                #searches for relevant property
                s1 = (meltsearch.find("Melting Point"))
                s2 = len("Melting Point")
                s3 = s1+s2
                for i in range(s3,len(meltsearch)):
                    if meltsearch[i] == 'C':
                        #parses the out the appropriate data from the unicode text
                        #sets data to GUI element
                            self.meltMessage=(meltsearch[i-8:i+1])
                            break
                break
        ...
        for x in range(0,len(ChemData)):
            if "Experimental Boiling Point" in ChemData[x]:
               ...
               for i in range(s3,len(boilsearch)):
                    if boilsearch[i] == 'C':
                        self.boilMessage=(boilsearch[i-8:i+1])
                        break
               break
...
class synthWindow(Frame):
    ...
    def assignment(self):
        ...
        #asks user for name of reactant and molar ratio of each reactant
    ...
    #utilizes appropriate molar ratios to generate a gram value for each reactant
    def calculate(self):
        print(ReactantDict)
